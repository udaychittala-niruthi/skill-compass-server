 ...
 */
const IconValidatorSeeder = async (_sequelize: any, transaction?: any) => {
    ...
    try {
        ...
        for (const skill of skills) {
            ...
            if (validated.icon !== original.icon || validated.iconLibrary !== original.iconLibrary) {
                ...
                console.log(
                    `  ✓ Fixed skill "${skill.name}": ${original.icon} (${original.iconLibrary}) → ${validated.icon} (${validated.iconLibrary})`
                );
                skillsFixed++;
            }
        }
        ...
        for (const interest of interests) {
            ...
            if (validated.icon !== original.icon || validated.iconLibrary !== original.iconLibrary) {
                ...
                console.log(
                    `  ✓ Fixed interest "${interest.name}": ${original.icon} (${original.iconLibrary}) → ${validated.icon} (${validated.iconLibrary})`
                );
                interestsFixed++;
            }
        }
        ...
        for (const course of courses) {
            ...
            if (validated.icon !== original.icon || validated.iconLibrary !== original.iconLibrary) {
                ...
                console.log(
                    `  ✓ Fixed course "${course.name}": ${original.icon} (${original.iconLibrary}) → ${validated.icon} (${validated.iconLibrary})`
                );
                coursesFixed++;
            }
        }
