...
class PredictionService {
    async predictCourses(data: PredictionRequest): Promise<PredictedCourse[]> {
        ...
        // 3. Call AI
        try {
            const aiResponse = await getJsonCompletion<{ id: number; score: number; reasoning: string }[]>(prompt, {
    ...
    async predictBranches(data: PredictionRequest): Promise<PredictedCourse[]> {
        ...
        try {
            ...
            // Handle potential wrapping
            const items = Array.isArray(aiResponse) ? aiResponse : (aiResponse as any).predictions || [];
