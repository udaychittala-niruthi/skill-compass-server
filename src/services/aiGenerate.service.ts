...
class AiGenerateService {
     ...
     */
    async syncCourses(inputCourses: SyncCourseInput[], generateCount: number = 0): Promise<SyncCourseResult> {
        const results: SyncCourseResult = { created: 0, updated: 0, deleted: 0, generated: 0 };
        ...
        const processedCourses: SyncCourseInput[] = [];
        const itemsToEnrich: SyncCourseInput[] = [];
        ...
        if (generateCount > 0) {
            ...
            const generated = await this.generateContextualCourses(
                skillNames,
                interestNames,
                knownNames,
                generateCount
    ...
    public async enrichCourseIcons(items: SyncCourseInput[]): Promise<SyncCourseInput[]> {
        const enriched: SyncCourseInput[] = [];
        const chunkSize = 20;
        ...
        for (let i = 0; i < items.length; i += chunkSize) {
            ...
            try {
                ...
                `;
                const response = await getJsonCompletion<{ items: SyncCourseInput[] }>(prompt);
                if (response?.items) enriched.push(...response.items);
            } catch (e) {
    ...
    private async generateContextualCourses(
        skills: string[],
        interests: string[],
        existingNames: string[],
        count: number
    ): Promise<SyncCourseInput[]> {
