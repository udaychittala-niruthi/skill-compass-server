...
export const compareImages = async (req, res) => {
    try {
        ...
        if (!files?.image1?.[0] || !files?.image2?.[0]) {
            return sendResponse(res, false, "Both image1 and image2 are required", 400);
        }
        ...
        if (!response.ok) {
            ...
            return sendResponse(res, false, "FastAPI error", response.status, null, errorData.detail || "Unknown error");
        }
        ...
        return sendResponse(res, false, error instanceof Error ? error.message : "Unknown error", 500);
    }
};
// Optional: API info endpoint
export const getApiInfo = (req, res) => {
    const info = {
        description: "Compare two images for similarity using CLIP (Contrastive Language-Image Pre-training)",
