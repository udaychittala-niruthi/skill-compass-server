 ...
 */
function isValidIcon(icon, library) {
 ...
 */
function getValidatedIcon(icon, library, itemType = "default") {
 ...
 */
export function validateSeederItems(items, itemType = "default") {
...
export async function resolveIconsAndGenerateNew(itemsToEnrich, knownItemNames, type, countToGenerate = 20) {
    const chunkSize = 30;
    const results = [];
...
// ... existing code ...
export async function generateCoursesContextual(skills, interests, existingCourseNames, countToGenerate = 20) {
    ...
    try {
        ...
        return validateSeederItems(items, "course");
    }
    catch (e) {
...
export async function resolveCoursesAndGenerateNew(itemsToEnrich, knownCourseNames, skills, interests, countToGenerate = 20) {
    const results = [];
    ...
    if (itemsToEnrich.length > 0) {
        ...
        for (let i = 0; i < itemsToEnrich.length; i += chunkSize) {
            ...
            try {
                ...
                if (response?.items) {
                    // Validate icons before adding
                    const validatedItems = validateSeederItems(response.items, "course");
                    results.push(...validatedItems);
                }
            }
            catch (e) {
