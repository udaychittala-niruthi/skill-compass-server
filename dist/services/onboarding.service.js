...
class OnboardingService {
    ...
    async onboardTeen(userId, data) {
        ...
        if (data.courseId !== undefined)
            prefsData.courseId = data.courseId > 0 ? data.courseId : null;
        if (data.branchId !== undefined)
            prefsData.branchId = data.branchId > 0 ? data.branchId : null;
        let prefs = await UserPreferences.findOne({ where: { userId } });
        if (!prefs) {
    ...
    async onboardStudent(userId, data) {
        const user = await User.findByPk(userId);
        if (!user)
            throw new Error("User not found");
    ...
    async onboardProfessional(userId, data) {
        const user = await User.findByPk(userId);
        if (!user)
            throw new Error("User not found");
        if (!data.courseId ||
    ...
    async onboardSenior(userId, data) {
        const user = await User.findByPk(userId);
        if (!user)
            throw new Error("User not found");
        ...
        if (data.courseId !== undefined)
            updateData.courseId = data.courseId > 0 ? data.courseId : null;
        if (data.branchId !== undefined)
            updateData.branchId = data.branchId > 0 ? data.branchId : null;
        if (!prefs) {
            prefs = await UserPreferences.create({
                userId,
                ...updateData
    ...
    async updateAge(userId, age) {
        ...
        if (age < 13)
            ...
            group = "SENIORS";
        await user.update({ age, group: group }); // Type assertion for group
        return { age, group };
    }
    ...
    async updateSkillsAndInterests(userId, data) {
        const user = await User.findByPk(userId);
        if (!user)
            throw new Error("User not found");
        if (user.age === null) {
        ...
        if (!prefs) {
            ...
            if (data.skillIds !== undefined)
                updateData.skillIds = data.skillIds;
            if (data.interestIds !== undefined)
                updateData.interestIds = data.interestIds;
            await prefs.update(updateData);
        }
        return prefs;
    }
    async getUserSkillsAndInterests(userId) {
        const user = await User.findByPk(userId);
        if (!user)
            throw new Error("User not found");
        if (user.age === null) {
