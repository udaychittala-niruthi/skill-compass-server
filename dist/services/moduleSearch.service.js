...
export class ModuleSearchService {
     ...
     */
    async findMatchingModules(criteria) {
        ...
        try {
            ...
            // Score each module
            const scoredModules = await Promise.all(modules.map(async (module) => this.scoreModuleQuality(module, courseId, branchId)));
            ...
            const rankedModules = this.rankModulesByRelevance(qualityModules.map((sm) => sm.module), { skills, interests: criteria.interestIds });
     ...
     */
    async scoreModuleQuality(module, targetCourseId, targetBranchId) {
        const breakdown = {
            hasResources: 0,
            averageRating: 0,
            completionCount: 0,
        ...
        if (module.averageRating !== null) {
            ...
            if (rating >= 4.5)
                breakdown.averageRating = 20;
            else if (rating >= 4.0)
                breakdown.averageRating = 15;
            else if (rating >= 3.5)
        ...
        if (completions >= 50)
            breakdown.completionCount = 15;
        else if (completions >= 20)
            breakdown.completionCount = 10;
        else if (completions >= 10)
            breakdown.completionCount = 5;
     ...
     */
    rankModulesByRelevance(modules, userContext) {
        return modules
            .map((module) => {
     ...
     */
    calculateReuseStats(totalNeeded, existingCount, newCount) {
        return {
     ...
     */
    getModuleTypeDistribution(modules) {
        const distribution = {};
     ...
     */
    balanceModuleTypes(existingModules, targetDistribution) {
        const balanced = [];
        const typeCounts = {};
