...
async function getAvailableModels() {
    ...
    try {
        ...
        const models = list.data.map((m) => m.id).filter((id) => typeof id === "string");
        if (models.length > 0) {
            cachedModels = models;
            lastFetchTime = now;
 ...
 */
async function withModelFallback(operation, preferredModel = DEFAULT_MODEL) {
    ...
    const allModels = [preferredModel, ...availableModels.filter((m) => m !== preferredModel)];
    // Filter out blocked models
    const modelsToTry = allModels.filter((m) => !blockedModels.has(m));
 ...
 */
export async function getChatCompletion(prompt, options = {}) {
...
export async function getJsonCompletion(prompt, options = {}) {
    const { temperature = 0.5, // Lower temperature for structured output stability
    max_tokens = 2048, systemPrompt = "You are a helpful assistant that outputs strictly in JSON format.", model: requestedModel } = options;
